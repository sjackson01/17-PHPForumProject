...
// This page shows the thread in a forum. 
include('inc/header.php');

// Retrieve all the messages in the forum 
...
$q = "SELECT t.thread_id, t.subject, username, COUNT(post_id) - 1 
      AS responses, MAX(DATE_FORMAT($last, '%e-%b-%y %l:%i %p')) 
      AS last, MIN(DATE_FORMAT($first,'%e-%b-%y %l:%i %p')) 
      AS first FROM threads 
      AS t INNER JOIN posts 
      AS p USING (thread_id) INNER JOIN users 
