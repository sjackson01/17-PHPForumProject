...
if (isset($_GET['tid']) && filter_var($_GET['tid'], FILTER_VALIDATE_INT, array( 'min_range' => 1))) {
    ...
    // Convert the date if the suer is logged in:
    ...
    // Run the query: 
    $q = "SELECT t.subject, p.message, username, 
          DATE_FORMAT($posted, '%e-%b-%y %l:%i %p') 
          AS posted FROM threads 
          AS t LEFT JOIN posts 
